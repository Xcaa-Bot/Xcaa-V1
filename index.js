...
async function startXcaa() {
    ...
    xcaa.on('group-participants-update', async (anu) => {
		   try {
			const mdata = await xcaa.groupMetadata(anu.id)
			console.log(anu)
			if (anu.action == 'add') {
				num = anu.participants[0]
            xcaa.sendMessage(mdata.id, {audio: fs.readFileSync('./sound/welcome.mp3'), contextInfo: { mentionedJid: [num] }, ptt:true})
         } else if (anu.action == 'remove') {
            num = anu.participants[0]
            xcaa.sendMessage(mdata.id, {audio: fs.readFileSync('./sound/leave.mp3'), contextInfo: { mentionedJid: [num] }, ptt:true})
         }
      } catch (err) {
         console.log(err)
     }
    ...
    xcaa.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            lastDisconnect.error?.output?.statusCode !== DisconnectReason.loggedOut ? startXcaa() : console.log('Connecting...')
        }
